...
const Header: React.FC = () => {
  ...
  const mobileLogoScale = useTransform(scrollY, [0, 25], [1, 0]);
  ...
  return (
    <>
      <header className="w-full bg-white sticky top-0 z-50 shadow-sm border-b border-gray-100">
        ...
        >
          {/* Left side - Logo (visible when scrolled) */}
          <motion.div
            style={{ opacity: mobileLeftLogoOpacity }}
            className={`absolute top-1/2 transform -translate-y-1/2 flex items-center flex-shrink-0 ${isArabic ? 'right-4' : 'left-4'}`}
          >
          ...
          {/* Center - Big Logo (visible when at top) */}
          ...
          <motion.div 
            className={`absolute top-1/2 transform -translate-y-1/2 flex items-center gap-1 ${isArabic ? 'left-4' : 'right-4'}`}
            style={{ 
          ...
          >
            ...
            >
              <ShoppingCart size={20} className="text-gray-700 group-hover:text-lion-500 transition-colors duration-300" />
              {totalItems > 0 && (
            ...
            <button 
              ...
              className="flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-2 text-sm hover:bg-gray-50 hover:border-lion-300 transition-all duration-300 hover:scale-105 font-semibold text-gray-700 hover:text-lion-500"
            >
              ...
              <div className="flex flex-col space-y-4">
                {smallNavItems.map((item) => (
                  <Link
                    ...
                    className="nav-link text-lg font-semibold text-gray-800 py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors duration-200"
                  >
                    {t(`nav.${item.key}`)}
                  </Link>
                ))}
